...
class Factorial(Operator):
    ...
    def execute(self, num1: float):
        if num1 < 0 or num1 is None:
            raise TypeError("in gamma you have to give a number tha bigget or equal to 0z")
        return float(math.gamma(num1))
...
class MathOperation:
    def __init__(self):
        ...
        self.gamma = Factorial()
        self.operators = {
            '!': self.gamma, '~': self.negative, '@': self.maximum, '&': self.minimum, '$': self.averge,
